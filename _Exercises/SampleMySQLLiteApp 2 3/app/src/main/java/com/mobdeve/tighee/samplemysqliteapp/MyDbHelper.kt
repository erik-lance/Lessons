...
class MyDbHelper(context: Context?) : SQLiteOpenHelper(context, DbReferences.DATABASE_NAME, null, DbReferences.DATABASE_VERSION) {
    ...
    fun editContact(c: Contact) {
        ...
        values.put(DbReferences.COLUMN_NAME_NUMBER, c.number)
        values.put(DbReferences.COLUMN_NAME_IMAGE_URI, c.imageUri)
        values.put(DbReferences._ID, c.id)

        database.update(DbReferences.TABLE_NAME, values, DbReferences._ID + " = ?", arrayOf(c.id.toString()))
        database.close()
    }
    ...
    fun deleteContact(c: Contact) {
        ...
        database.delete(DbReferences.TABLE_NAME, DbReferences._ID + " = ?", arrayOf(c.id.toString()))
        database.close()
    }
